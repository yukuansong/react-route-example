...
var apigatewayendpoint = 'https://pajxfiaq35.execute-api.us-east-2.amazonaws.com/dev';
  ...
  if (query.length > 2) {
    ...
    if (results.length > 0) {
      ...
      for (var item in results) {
        ...
        let description = results[item]._source.Description;
        let doctor = results[item]._source.Docter;
        let patient = results[item]._source.Patient;
        // Construct the full HTML string that we want to append to the div
        resultdiv.append('<div class="result">' +
        '<h2 style="width: 50%">' + 'Patient:' + patient + '</h2>' +
        '<h2 style="width: 50%">' + 'Doctor:' +doctor + '</h2></div>');
        // resultdiv.append('<div class="result">' +
